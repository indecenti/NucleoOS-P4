...
// setup with a live key test — all browser-direct (the device is never loaded). Resolves when the
  ...
  return new Promise((resolve) => {
    ...
    function renderWelcome() {
      card.append(
        header(en ? 'Welcome to NucleoOS' : 'Benvenuto in NucleoOS',
               en ? 'A web-native OS that lives on your device' : 'Un OS web-native che vive sul tuo device'),
        h('div', { class: 'nob-body' },
          h('p', { html: en
            ? 'NucleoOS turns your device into a tiny computer you drive from this browser. At its heart is <b>ANIMA</b> — an assistant whose brain runs <b>offline, on the device itself</b>.'
            : 'NucleoOS trasforma il tuo device in un piccolo computer che guidi da questo browser. Al centro c’è <b>ANIMA</b> — un assistente il cui cervello gira <b>offline, sul dispositivo stesso</b>.' }),
          h('div', { class: 'nob-feat' },
            ...
            feat('⚡', en ? 'Light on the device' : 'Leggero sul dispositivo', en ? 'The heavy AI work runs in your browser, not the chip.' : 'Il lavoro pesante dell’IA gira nel browser, non sul chip.')),
    ...
    function renderInstall() {
      ...
      const body = h('div', { class: 'nob-body' },
        h('p', { html: en
          ? 'NucleoOS is served by the device over your network — no app store. <b>Install it</b> to get a full-screen app icon and instant launch.'
          : 'NucleoOS è servito dal dispositivo sulla tua rete — niente app store. <b>Installalo</b> per avere un’icona a tutto schermo e avvio immediato.' }),
        h('ol', { class: 'nob-steps' },
          li(en ? 'Keep the device on the same Wi-Fi as this browser.' : 'Tieni il dispositivo sulla stessa Wi-Fi di questo browser.'),
    ...
    function renderProvider() {
      ...
      card.append(
        ...
        h('div', { class: 'nob-body' }, h('div', { class: 'nob-choices' },
          mk('anthropic', '🟣', 'Claude (Anthropic)', en ? 'Best understanding. Runs in your browser — zero load on the device.' : 'Comprensione al top. Gira nel browser — zero carico sul dispositivo.', true),
    ...
    function renderKey() {
      ...
      const where = state.provider === 'anthropic'
        ...
        ? [li(en ? 'Go to ' : 'Vai su ', h('a', { href: 'https://aistudio.google.com/apikey', target: '_blank', rel: 'noopener' }, 'aistudio.google.com/apikey')),
           ...
           li(en ? 'The device relays the call (Google has no browser access).' : 'Il device inoltra la chiamata (Google non è accessibile dal browser).')]
      ...
      card.append(
        ...
        h('div', { class: 'nob-body' },
          ...
          h('div', { class: 'nob-note', html: '🔒 ' + (en
            ? 'Stored only on your device (SD), never logged. With Claude the key stays in <b>your browser</b> and talks directly to Anthropic — the device isn’t loaded.'
            : 'Salvata solo sul tuo dispositivo (SD), mai nei log. Con Claude la chiave resta nel <b>tuo browser</b> e parla direttamente con Anthropic — il dispositivo non viene caricato.') })),
        footer({ back: true, skip: true, saveSlot: saveBtn }));
    }

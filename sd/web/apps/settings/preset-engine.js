...
//      the writes/apiCalls to run. The caller runs them one at a time (the device is single-task). This
...
export const PRESETS = [
  {
    ...
    plan(sig, reg) {
      ...
      return mk('device', 'on', 'auto', false, { using: { it: 'Cervello del device', en: 'The device brain' } });
    },
    feasible() { return { state: 'recommended', reason: { it: '', en: '' } }; },
  },
  {
    id: 'max', icon: '🚀', it: 'Massima qualità', en: 'Max quality',
    intent: { it: 'Cloud al massimo, il dispositivo resta a riposo', en: 'Best cloud, the device stays idle' },
    plan(sig, reg) {
  ...
  },
  {
    ...
    intent: { it: 'Tutto nel browser, il dispositivo può dormire', en: 'All in the browser, the device can sleep' },
    plan(sig) {
      if (sig.webgpu && (sig.vramMB || 0) >= 900 && sig.localModelReady)
        return mk('gpu', 'local', 'auto', true, { localModel: sig.localModelId || 'auto', using: { it: 'LLM nella GPU del browser', en: 'LLM on the browser GPU' } });
      if (sig.packUsable)

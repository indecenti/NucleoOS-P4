// constellations.js — "Costellazioni 3D": the web Game Center continuation of the device space
...
// the save — so the campaign is shared both ways with the device.
//
...
async function bootRun() {
  RUN = await loadSave();                 // continue the device run, or null -> offer a New Run
  BOOTED = true;
...
// ---- combat tuning (mirrors the device rail shooter, in normalized space) -------------------
...
export default defineGame({
  ...
  tick(state, dtMs) {
    ...
    if (conflictPending && state.phase === 'hub') { conflictPending = null; return { ...state, phase: 'conflict', toast: { text: 'Il device ha fatto avanzare la run', kind: 'warn' } }; }

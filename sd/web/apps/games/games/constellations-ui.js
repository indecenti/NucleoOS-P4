...
export function makeUI(canvas) {
  ...
  function paint(m, st) {
    ...
    if (ph === 'new_run') { $('[data-modal]').innerHTML = `<div class="cz-panel cz-card"><h3>Nessuna run da continuare</h3><div style="color:#9fb0c4;max-width:360px">Avvia una nuova run web. Verra' sincronizzata col device al primo salvataggio.</div><div class="cz-btn cz-row sel" data-row="0">Inizia</div></div>`; return; }
    if (ph === 'conflict') { $('[data-modal]').innerHTML = `<div class="cz-panel cz-card"><h3 style="color:#ffbe40">Run avanzata su device</h3><div style="color:#9fb0c4;max-width:360px">Ho ricaricato l'ultimo stato dal dispositivo (settore ${r ? r.sector : 0}, ${r ? r.credits : 0} cr).</div><div class="cz-btn cz-row sel" data-row="0">OK</div></div>`; return; }
    if (!r) return;

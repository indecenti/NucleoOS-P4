...
void w4_framebufferOval (int x, int y, int width, int height) {
    ...
    // NucleoV2: 64-bit decision parameters (the int originals overflow past ~800 px).
    const int64_t a2 = (int64_t)a * a;
    const int64_t b2 = (int64_t)b * b;

    int64_t dx = 4 * (int64_t)(1 - a) * b2;
    int64_t dy = 4 * (int64_t)(b1 + 1) * a2;

    // Error of 1 step
    int64_t err = dx + dy + b1 * a2;

    const int64_t a8 = 8 * a2;
    const int64_t b8 = 8 * b2;
...
void w4_framebufferTextUtf16 (const uint16_t* str, int byteLength, int x, int y) {
    // NucleoV2: byte-wise NUL test — a cart may pass an odd address, and a direct *str load of an
    // unaligned uint16_t can fault on the host CPU.
    for (int currentX = x; byteLength > 0 && w4_read16LE(str); ++str, byteLength -= 2) {

...
Target : components/nv_anima/nucleo_anima.c  (block between the
         `// <gen:app-alias>` / `// </gen:app-alias>` markers)

Ported from G:\\Nucleo tools/anima/gen_aliases.py; NucleoV2 has no registry/apps.json
(native apps register in C, WASM apps come from the SD), so the installed-app check
is skipped when that file is absent.
...
Validation (all run every time):
  ...
  * a prefix collision per the firmware a_match() rule is a WARNING (order resolves it,
    but it's surfaced so the chosen order is a deliberate decision)
...
REG = ROOT / "registry" / "apps.json"          # optional on NucleoV2
TARGET = ROOT / "components" / "nv_anima" / "nucleo_anima.c"
BEGIN = "// <gen:app-alias>"
END = "// </gen:app-alias>"
...
def validate(aliases: dict, installed: set) -> tuple[list, list]:
    ...
    for app, al in aliases.items():
        if installed and app not in installed:
